...
select 
    ...
    agent_sales.2025_sales,
    round((agent_sales.total / agent_sales.2025_sales), 2) as avg_price,
    cast(replace(replace(m.price_closed_median, '$', ''), ',', '') as signed) as market_median, 
    round(((agent_sales.total / agent_sales.2025_sales)
...
where assigned_agent = 'yuki_lewis_420'
